...
public final class MazeIO {
  ...
  public record WeightedMaze(GridGraph2D maze, int[] weights, int s, int t) {}

  public static void write(GridGraph2D maze, int[] weights, File file) {
    write(maze, weights, 0, maze.nbVertices() - 1, file);
  }

  public static void write(GridGraph2D maze, int[] weights, int s, int t, File file) {
    try (var writer = new BufferedWriter(new FileWriter(file))) {
      writer.write(maze.width() + " ");
      writer.write(maze.height() + "\n");
      writer.write(s + " " + t + "\n");
      Edge[] edges = maze.edges();
  ...
  public static WeightedMaze read(File file) {
    try (var reader = new Scanner(new BufferedReader(new FileReader(file)))) {
      ...
      return new WeightedMaze(maze, weights, s, t);
    } catch (IOException e) {
      throw new RuntimeException(e);
    }
  }
